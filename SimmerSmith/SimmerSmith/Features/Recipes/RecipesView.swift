...
struct RecipesView: View {
    ...
    var body: some View {
        ...
        .toolbar {
            ...
            if isSelecting {
                ...
                // SP-C review finding D: the top-bar sparkle opens the Fly-backed AI
                // assistant. Hide it in CloudKit-only mode (no token → 401).
                if forgePrimary != .sparkle && !appState.isCloudKitOnly {
                    ToolbarItem(placement: .topBarTrailing) {
    ...
    private var forgeAddMenu: some View {
        Menu {
            ...
            // SP-C AI-2: "Identify ingredient" is rewired to aiSvc.generateVision
            // (BYO key); un-gated like the import / web-search items above.
            Button {
