...
from . import mcp
from ._helpers import _call_route, _current_user
...
def weeks_list(limit: int = 6) -> list[dict[str, Any]]:
    with session_scope() as session:
        return _call_route(lambda: week_list(limit=limit, session=session, current_user=_current_user(session)))


@mcp.tool(description="Get the current week.")
def weeks_get_current() -> dict[str, Any] | None:
    with session_scope() as session:
        return _call_route(lambda: current_week(session=session, current_user=_current_user(session)))


@mcp.tool(description="Get a week by week start date (YYYY-MM-DD).")
def weeks_get_by_start(week_start: str) -> dict[str, Any] | None:
    with session_scope() as session:
        return _call_route(lambda: week_by_start(week_start=week_start, session=session, current_user=_current_user(session)))


@mcp.tool(description="Get a week by ID.")
def weeks_get(week_id: str) -> dict[str, Any]:
    with session_scope() as session:
        return _call_route(lambda: week_detail(week_id, session=session, current_user=_current_user(session)))
...
def weeks_create(week_start: str, notes: str = "") -> dict[str, Any]:
    with session_scope() as session:
        ...
        return _call_route(lambda: create_week(payload, session=session, current_user=_current_user(session)))
...
def weeks_apply_ai_draft(
    ...
    with session_scope() as session:
        ...
        return _call_route(lambda: apply_draft(week_id, payload, session=session, current_user=_current_user(session)))
...
def weeks_update_meals(week_id: str, meals: list[dict[str, Any]]) -> dict[str, Any]:
    with session_scope() as session:
        ...
        return _call_route(lambda: update_meals(week_id, payload, session=session, current_user=_current_user(session)))


@mcp.tool(description="Get change history for a week.")
def weeks_get_changes(week_id: str) -> list[dict[str, Any]]:
    with session_scope() as session:
        return _call_route(lambda: week_changes(week_id, session=session, current_user=_current_user(session)))


@mcp.tool(description="Mark a week ready for AI review.")
def weeks_mark_ready_for_ai(week_id: str) -> dict[str, Any]:
    with session_scope() as session:
        return _call_route(lambda: ready_for_ai(week_id, session=session, current_user=_current_user(session)))


@mcp.tool(description="Approve a week.")
def weeks_approve(week_id: str) -> dict[str, Any]:
    with session_scope() as session:
        return _call_route(lambda: approve_week(week_id, session=session, current_user=_current_user(session)))


@mcp.tool(description="Regenerate the grocery list for a week.")
def weeks_regenerate_grocery(week_id: str) -> dict[str, Any]:
    with session_scope() as session:
        return _call_route(lambda: regenerate_grocery(week_id, session=session, current_user=_current_user(session)))


@mcp.tool(description="Get saved feedback for a week.")
def weeks_get_feedback(week_id: str) -> dict[str, Any]:
    with session_scope() as session:
        return _call_route(lambda: week_feedback(week_id, session=session, current_user=_current_user(session)))
...
def weeks_save_feedback(week_id: str, entries: list[dict[str, Any]]) -> dict[str, Any]:
    with session_scope() as session:
        ...
        return _call_route(lambda: save_week_feedback(week_id, payload, session=session, current_user=_current_user(session)))


@mcp.tool(description="Get pricing results for a week.")
def weeks_get_pricing(week_id: str) -> dict[str, Any]:
    with session_scope() as session:
        return _call_route(lambda: pricing_detail(week_id, session=session, current_user=_current_user(session)))
...
def weeks_import_pricing(
    ...
    with session_scope() as session:
        ...
        return _call_route(lambda: import_week_pricing(week_id, payload, session=session, current_user=_current_user(session)))


@mcp.tool(description="List export runs for a week.")
def weeks_list_exports(week_id: str) -> list[dict[str, Any]]:
    with session_scope() as session:
        return _call_route(lambda: week_exports(week_id, session=session, current_user=_current_user(session)))
...
def weeks_create_export(week_id: str, destination: str, export_type: str) -> dict[str, Any]:
    with session_scope() as session:
        ...
        return _call_route(lambda: create_week_export(week_id, payload, session=session, current_user=_current_user(session)))
