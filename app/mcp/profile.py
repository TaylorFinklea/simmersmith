...
def profile_get() -> dict[str, Any]:
    with session_scope() as session:
        return _call_route(lambda: get_profile(session=session, current_user=_current_user(session)))
...
def profile_update(
    ...
    with session_scope() as session:
        ...
        return _call_route(lambda: put_profile(payload, session=session, current_user=_current_user(session)))


@mcp.tool(description="Get meal preference signals and the summarized preference context.")
def preferences_get() -> dict[str, Any]:
    with session_scope() as session:
        return _call_route(lambda: get_preferences(session=session, current_user=_current_user(session)))
...
def preferences_upsert(signals: list[dict[str, Any]]) -> dict[str, Any]:
    with session_scope() as session:
        ...
        return _call_route(lambda: post_preferences(payload, session=session, current_user=_current_user(session)))
...
def preferences_score_meal(
    ...
    with session_scope() as session:
        ...
        return _call_route(lambda: post_score_meal(payload, session=session, current_user=_current_user(session)))
