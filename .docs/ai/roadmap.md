...
- [ ] **F23/F24 — IAP replay/dedup.** Low-risk now F22 closes forgery. Needs iOS `appAccountToken` set at purchase + matched server-side, plus a `notificationUUID` dedup table + `signedDate` freshness on the webhook. Tier: Opus (iOS + migration).
...
- [ ] **Mediums worth a security pass:** pin session-JWT algorithm (`app/auth.py`); `jwt_secret` strength check (`app/config.py`); `verify_state` aud/iss + `email_verified` (`app/services/sso.py`); whitespace-token free-tier bypass (`app/services/entitlements.py`); Tier: Haiku/Sonnet each.
