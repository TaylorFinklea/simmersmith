...
- **F28 — recipe-import SSRF → FIXED (`e71195b`, hardened `a76e61c`).** Resolves the host and rejects any private/loopback/link-local/reserved A/AAAA; redirects followed manually with per-hop revalidation. Follow-up (per security review): **pins the validated IP into the connection** (`_pinned_connect_url` + Host header + `sni_hostname` extension for correct TLS) to close the DNS-rebinding TOCTOU between validate and connect. Regression tests.
