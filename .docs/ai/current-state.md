...
- [ ] **e0a P2e test-only cached app boot/state.** `tier_floor: senior` · `complexity: L`. Verify: CloudKit package + ad-hoc app-target suite + generic iOS build command in P2 spec §9. Caveat: read-only-participant checkpoints carry zoneEnsured=false → engine seam rejects+quarantines every launch; demote to recovery-only at the catalog or revisit participant semantics (decisions.md 2026-07-17 flag).
