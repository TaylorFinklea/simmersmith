...
- [ ] **e0a P1b persistence contracts/archive.** Implement spec §6 P1b only: immutable scope/manifest/envelopes/outbox/receipt/canonical logical digest; secure full-CKRecord round trip; generation-local asset rebinding; unknown assets cannot publish; no engine/app edits. `tier_floor: senior` · `complexity: L`. Verify: `swift test --package-path SimmerSmithCloudKit`.
