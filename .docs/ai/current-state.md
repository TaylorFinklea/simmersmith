...
- [ ] **e0a P1c transactional writer/recovery.** Implement spec §6 P1c only: framed fsynced WAL, immutable record-first/state-second generations, atomic `current`, replay above manifest high-water, exact ack/tombstone handling, failpoints/quarantine; no engine/session/app edits. `tier_floor: lead` · `complexity: XL`. Verify: `swift test --package-path SimmerSmithCloudKit`.
