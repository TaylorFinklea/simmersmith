...
- [ ] Phase 3 device gate (HUMAN — not looped): install a live-key build; confirm tokens VISIBLY stream across OpenAI / Anthropic / each open model + open-models reasoning replay holds across a multi-iteration tool loop. → harness-deck `simmersmith/assistant-streaming-device-test`.
